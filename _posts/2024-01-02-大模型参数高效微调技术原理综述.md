...
![img](/img/posts/LLM-PEFT/peft_type.png?raw=true)
...
- ![img](/img/posts/LLM-PEFT/lora_main_method.jpg?raw=true)
- 技术原理：
...
- 可以通过可插拔的形式切换到不同的任务
- ![img](/img/posts/LLM-PEFT/lora_example.png?raw=true)
...
- ![img](/img/posts/LLM-PEFT/mam_adapter_example.jpg?raw=true)
- ![img](/img/posts/LLM-PEFT/mam_adapter_experiment.png?raw=true)
- **具体实现**
...
- **技术原理**
  ...
  - ![img](/img/posts/LLM-PEFT/unipeft.jpg?raw=true)
- 对不同 PELT 方法训练时间和推理时间进行了分析
  ...
  - ![img](/img/posts/LLM-PEFT/unipeft_experiment.png?raw=true)
...
![img](/img/posts/LLM-PEFT/conclusion-1.png?raw=true)

下表展示了各种参数高效方法的参与训练的参数量、最终模型与原始模型的改变参数（delta值）以及论文中参与评估的模型的范围（<1B、<20B、>20B）。

![img](/img/posts/LLM-PEFT/conclusion-2.png?raw=true)
...
- 奇异值（SVD）分解：AdaLoRA
- CP 分解( Canonical Polyadic Decomposition (CPD)：![深入理解 | CP、Tucker分解](/img/posts/LLM-PEFT/cpd.png?raw=true)
- **Tucker分解**：计算复杂度比较大
  - ![img](/img/posts/LLM-PEFT/tuckerd.png?raw=true)
...
![img](/img/posts/LLM-PEFT/aurora.jpeg?raw=true)

- Aurora
- 采用模式近似（mode apprximation）的方法来进行大模型的轻量化高效微调，仅需训练预训练大模型0.04%的参数
- 架构原理：CP分解（秩为R）
  - ![img](/img/posts/LLM-PEFT/aurora-cpd.png?raw=true)
...
- 实验结果
  - 图文检索
    - ![img](/img/posts/LLM-PEFT/aurora-itr.png?raw=true)
  - 视频文本检索
    - ![img](/img/posts/LLM-PEFT/aurora-vtr.png?raw=true)
  - VQA
    - ![img](/img/posts/LLM-PEFT/aurora-vqa.png?raw=true)
